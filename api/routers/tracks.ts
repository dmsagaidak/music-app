...
tracksRouter.get('/', userMiddleware, async (req, res, next) => {
  try {
    ...
    if (!user || user.role === 'user') {
      if (req.query.album) {
        const tracks = await Track.find({ album: req.query.album, isPublished: true })
          .populate('album')
          .sort({ tracknumber: 1 });
        return res.send(tracks);
      } else if (req.query.artist) {
        const albums = await Album.find({ artist: req.query.artist });
        const albumIds = albums.map((item) => {
          return item._id;
        });
...
tracksRouter.patch('/:id/togglePublished', auth, permit('admin'), async (req, res, next) => {
  try {
    ...
    if (updatingTrack.isPublished) {
      await Track.updateOne({ _id: req.params.id }, { $set: { isPublished: false } });
    } else {
      await Track.updateOne({ _id: req.params.id }, { $set: { isPublished: true } });
    }
