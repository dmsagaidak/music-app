    ...
    <Grid container direction="column" spacing={2}>
      {trackHistory.length ? (
        ...
        <>
          ...
          <Typography component={'a'} href={frontUrl} sx={{ ml: 2 }} style={{ textDecoration: 'none' }}>
            It&apos;s time to listen to some music
          </Typography>
        </>
      )}

      {trackHistoryFetching ? (
        <Progress />
      ) : (
        trackHistory.map((item) => (
          <Card key={item._id} sx={{ m: 1 }}>
            <Grid container direction="row" justifyContent="space-around" alignItems="center">
              <Typography component={'span'} style={{ paddingLeft: '10px', width: '200px' }}>
                {item.track.title}
              </Typography>
              <Typography component={'span'} width={'200px'}>
                by {item.artist.name}
              </Typography>
