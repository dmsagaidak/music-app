    ...
    <>
      <Card sx={{ mb: 1 }} key={track._id}>
        <Grid container direction="row" justifyContent="space-around" alignItems="center">
          <Typography component="span" style={{ paddingLeft: '9px', paddingRight: '9px', width: '15px' }}>
            {track.tracknumber}
          </Typography>
          {user ? (
            <Typography component={'div'} width={'100px'}>
              <IconButton
                type="button"
              ...
              {track.video ? (
                ...
                <Typography component={'div'} width={'40px'}></Typography>
              )}
            </Typography>
          ) : (
            <Typography component={'div'} width={'100px'}></Typography>
          )}
          <Typography component={'span'} width={'550px'}>
            {track.title}
          </Typography>
          <Typography component={'span'}>{track.duration}</Typography>
          {user?.role === 'admin' && (
            <>
              <Button
                type="button"
        ...
        {!track.isPublished && (
          <Typography component="span" width="100px" color="red" textAlign="center">
            Unpublished
          </Typography>
        )}
      </Card>
      <Modal open={open} onClose={() => setOpen(false)}>
        <Box position={'absolute'} top={'30%'} left={'30%'}>
          {track.video ? <ReactPlayer url={track.video} /> : <Typography component="div"></Typography>}
        </Box>
      </Modal>
    </>
  );
